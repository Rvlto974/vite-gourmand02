...
class AdminController {
    ...
    public function modifierMenu() {
        $this->verifierAdmin();
        $id = $_GET['id'] ?? null;
        if (!$id) { header('Location: /admin/menus'); exit; }
        $menu = $this->menuModel->getById($id);
        if (!$menu) { header('Location: /admin/menus'); exit; }

        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            // Modifier les infos du menu
            ...
            $this->menuModel->supprimerImages($id);
            $images = $_POST['images'] ?? [];
            foreach ($images as $ordre => $url) {
                if (!empty(trim($url))) {
                    $this->menuModel->ajouterImage($id, trim($url), $ordre);
                }
            }

            // Gérer les plats
            $db = (new Database())->connect();
            $db->prepare("DELETE FROM plats WHERE menu_id = ?")->execute([$id]);
            $plats = $_POST['plats'] ?? [];
            foreach ($plats as $plat) {
                if (!empty(trim($plat['nom']))) {
                    $stmt = $db->prepare("INSERT INTO plats (menu_id, nom, type, description, allergenes) VALUES (?,?,?,?,?)");
                    $stmt->execute([
                        $id,
                        trim($plat['nom']),
                        $plat['type'] ?? 'plat',
                        trim($plat['description'] ?? ''),
                        trim($plat['allergenes'] ?? ''),
                    ]);
                }
            }
    ...
    public function creerMenu() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $menuId = $this->menuModel->getLastInsertId();

            // Images
            $images = $_POST['images'] ?? [];
            foreach ($images as $ordre => $url) {
                if (!empty(trim($url))) {
                    $this->menuModel->ajouterImage($menuId, trim($url), $ordre);
                }
            }
